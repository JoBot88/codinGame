...
template<typename Tkey>
void display(const std::vector<Tkey> & my_heap);
/*
    Aim : display the heap to look like that
                                                                            key_1

                                                                key_2                   key_3
                                                    
                                                            key_4    key_5          key_6    key_7
    Args : heap to display
    Return value : None
    Complexy : O(log(n))
*/
...
void percolate_up(std::vector<Tkey>& my_heap, const Tnode n_start, bool min){      // performs differently for a min-heap and a max-heap
    ...
    if(min){
        while(current_node != 1 && my_heap[current_node] < my_heap[daddy(current_node)]){     // if we haven't reached the root and we have to swap with daddy 
    ...
    */
    ...
    if(right_son(n) < my_heap.size()){
        if(min){
            ...
            Tnode max_child = (my_heap[left_son(n)] > my_heap[right_son(n)]) ? left_son(n) : right_son(n);  // we swap with the min child
...
template<typename Tkey, typename Tnode>
void percolate_down(std::vector<Tkey>& my_heap, const Tnode n_start, const bool min){
    Tnode current_node(n_start);
...
template<typename Tkey, typename Tnode>
std::vector<Tkey> heapify(const std::vector<Tkey> & my_vector, const bool min){
...
template<typename Tkey>
void display(const std::vector<Tkey> & my_heap){
    unsigned int depth = ceil(log2(my_heap.size()-1));
    for(size_t i = 0; i < depth; i++){
        for(size_t k = 0; k < floor(pow(2, depth-i)/2); k++)
            std::cout << "\t" ;
        for(size_t j = 0; j < pow(2.0, i); j++){
            if(my_heap[pow(2.0, i) + j] != 0){
                std::cout << my_heap[pow(2.0, i) + j];
                for(size_t l = 0; l < 2*floor(pow(2, depth-i)/2); l++)
                    std::cout << "\t";
            }
